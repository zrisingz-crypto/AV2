...
#define MASKSADMXN_SSSE3(m, n)                                                \
  unsigned int avm_masked_sad##m##x##n##_ssse3(                               \
...
#define MASKSAD8XN_SSSE3(n)                                                   \
  unsigned int avm_masked_sad8x##n##_ssse3(                                   \
      const uint8_t *src, int src_stride, const uint8_t *ref, int ref_stride, \
      const uint8_t *second_pred, const uint8_t *msk, int msk_stride,         \
      int invert_mask) {                                                      \
    if (!invert_mask)                                                         \
      return avm_masked_sad8xh_ssse3(src, src_stride, ref, ref_stride,        \
                                     second_pred, 8, msk, msk_stride, n);     \
    else                                                                      \
      return avm_masked_sad8xh_ssse3(src, src_stride, second_pred, 8, ref,    \
                                     ref_stride, msk, msk_stride, n);         \
  }

#define MASKSAD4XN_SSSE3(n)                                                   \
  unsigned int avm_masked_sad4x##n##_ssse3(                                   \
      const uint8_t *src, int src_stride, const uint8_t *ref, int ref_stride, \
      const uint8_t *second_pred, const uint8_t *msk, int msk_stride,         \
      int invert_mask) {                                                      \
    if (!invert_mask)                                                         \
      return avm_masked_sad4xh_ssse3(src, src_stride, ref, ref_stride,        \
                                     second_pred, 4, msk, msk_stride, n);     \
    else                                                                      \
      return avm_masked_sad4xh_ssse3(src, src_stride, second_pred, 4, ref,    \
                                     ref_stride, msk, msk_stride, n);         \
  }
...
unsigned int avm_masked_sad4xh_ssse3(const uint8_t *src_ptr, int src_stride,
  ...
  for (y = 0; y < height; y += 2) {
    ...
    pred_16bit = xx_roundn_epu16(pred_16bit, AVM_BLEND_A64_ROUND_BITS);
...
#define HIGHBD_MASKSADMXN_SSSE3(m, n)                                        \
  unsigned int avm_highbd_masked_sad##m##x##n##_ssse3(                       \
...
#define HIGHBD_MASKSAD4XN_SSSE3(n)                                            \
  unsigned int avm_highbd_masked_sad4x##n##_ssse3(                            \
      const uint16_t *src, int src_stride, const uint16_t *ref,               \
      int ref_stride, const uint16_t *second_pred, const uint8_t *msk,        \
      int msk_stride, int invert_mask) {                                      \
    if (!invert_mask)                                                         \
      return avm_highbd_masked_sad4xh_ssse3(src, src_stride, ref, ref_stride, \
                                            second_pred, 4, msk, msk_stride,  \
                                            n);                               \
    else                                                                      \
      return avm_highbd_masked_sad4xh_ssse3(src, src_stride, second_pred, 4,  \
