...
#ifndef AVM_AVM_DSP_ARM_BLEND_NEON_H_
#define AVM_AVM_DSP_ARM_BLEND_NEON_H_
...
static INLINE uint8x16_t alpha_blend_a64_u8x16(uint8x16_t m, uint8x16_t a,
                                               uint8x16_t b) {
  const uint8x16_t m_inv = vsubq_u8(vdupq_n_u8(AVM_BLEND_A64_MAX_ALPHA), m);
  ...
  uint8x8_t blend_u8_lo = vrshrn_n_u16(blend_u16_lo, AVM_BLEND_A64_ROUND_BITS);
  uint8x8_t blend_u8_hi = vrshrn_n_u16(blend_u16_hi, AVM_BLEND_A64_ROUND_BITS);
  return vcombine_u8(blend_u8_lo, blend_u8_hi);
}

static INLINE uint8x8_t alpha_blend_a64_u8x8(uint8x8_t m, uint8x8_t a,
                                             uint8x8_t b) {
  const uint8x8_t m_inv = vsub_u8(vdup_n_u8(AVM_BLEND_A64_MAX_ALPHA), m);
  uint16x8_t blend_u16 = vmull_u8(m, a);
  blend_u16 = vmlal_u8(blend_u16, m_inv, b);
  return vrshrn_n_u16(blend_u16, AVM_BLEND_A64_ROUND_BITS);
}

static INLINE uint16x8_t alpha_blend_a64_u16x8(uint16x8_t m, uint16x8_t a,
                                               uint16x8_t b) {
  uint16x8_t m_inv = vsubq_u16(vdupq_n_u16(AVM_BLEND_A64_MAX_ALPHA), m);
  ...
  uint16x4_t blend_u16_lo =
      vrshrn_n_u32(blend_u32_lo, AVM_BLEND_A64_ROUND_BITS);
  uint16x4_t blend_u16_hi =
      vrshrn_n_u32(blend_u32_hi, AVM_BLEND_A64_ROUND_BITS);
  return vcombine_u16(blend_u16_lo, blend_u16_hi);
}

static INLINE uint16x4_t alpha_blend_a64_u16x4(uint16x4_t m, uint16x4_t a,
                                               uint16x4_t b) {
  const uint16x4_t m_inv = vsub_u16(vdup_n_u16(AVM_BLEND_A64_MAX_ALPHA), m);
  uint32x4_t blend_u16 = vmull_u16(m, a);
  blend_u16 = vmlal_u16(blend_u16, m_inv, b);
  return vrshrn_n_u32(blend_u16, AVM_BLEND_A64_ROUND_BITS);
}
...
#endif  // AVM_AVM_DSP_ARM_BLEND_NEON_H_
