...
void avm_transpose_float_sse2(const float *A, float *B, int n) {
  for (int y = 0; y < n; y += 4) {
    for (int x = 0; x < n; x += 4) {
...
void avm_fft_unpack_2d_output_sse2(const float *packed, float *output, int n) {
  const int n2 = n / 2;
  output[0] = packed[0];
  output[1] = 0;
...
void avm_fft4x4_float_sse2(const float *input, float *temp, float *output) {
  avm_fft_2d_gen(input, temp, output, 4, avm_fft1d_4_sse2,
                 avm_transpose_float_sse2, avm_fft_unpack_2d_output_sse2, 4);
}

void avm_fft8x8_float_sse2(const float *input, float *temp, float *output) {
  avm_fft_2d_gen(input, temp, output, 8, avm_fft1d_8_sse2,
                 avm_transpose_float_sse2, avm_fft_unpack_2d_output_sse2, 4);
}

void avm_fft16x16_float_sse2(const float *input, float *temp, float *output) {
  avm_fft_2d_gen(input, temp, output, 16, avm_fft1d_16_sse2,
                 avm_transpose_float_sse2, avm_fft_unpack_2d_output_sse2, 4);
}

void avm_fft32x32_float_sse2(const float *input, float *temp, float *output) {
  avm_fft_2d_gen(input, temp, output, 32, avm_fft1d_32_sse2,
                 avm_transpose_float_sse2, avm_fft_unpack_2d_output_sse2, 4);
}
...
void avm_ifft4x4_float_sse2(const float *input, float *temp, float *output) {
  avm_ifft_2d_gen(input, temp, output, 4, avm_fft1d_4_float, avm_fft1d_4_sse2,
                  avm_ifft1d_4_sse2, avm_transpose_float_sse2, 4);
}

void avm_ifft8x8_float_sse2(const float *input, float *temp, float *output) {
  avm_ifft_2d_gen(input, temp, output, 8, avm_fft1d_8_float, avm_fft1d_8_sse2,
                  avm_ifft1d_8_sse2, avm_transpose_float_sse2, 4);
}

void avm_ifft16x16_float_sse2(const float *input, float *temp, float *output) {
  avm_ifft_2d_gen(input, temp, output, 16, avm_fft1d_16_float,
                  avm_fft1d_16_sse2, avm_ifft1d_16_sse2,
                  avm_transpose_float_sse2, 4);
}

void avm_ifft32x32_float_sse2(const float *input, float *temp, float *output) {
  avm_ifft_2d_gen(input, temp, output, 32, avm_fft1d_32_float,
                  avm_fft1d_32_sse2, avm_ifft1d_32_sse2,
                  avm_transpose_float_sse2, 4);
}
