...
#
if(AVM_BUILD_CMAKE_COMPILER_TESTS_CMAKE_)
  return()
endif() # AVM_BUILD_CMAKE_COMPILER_TESTS_CMAKE_
set(AVM_BUILD_CMAKE_COMPILER_TESTS_CMAKE_ 1)
...
# $CMAKE_BUILD_TYPE is used. For both cases AVM_<LANG>_CONFIGS is populated with
# CMake string variable names that contain flags for the currently available
# configuration(s).
unset(AVM_C_CONFIGS)
unset(AVM_CXX_CONFIGS)
list(LENGTH CMAKE_CONFIGURATION_TYPES num_configs)
if(${num_configs} GREATER 0)
  foreach(config ${CMAKE_CONFIGURATION_TYPES})
    string(TOUPPER ${config} config)
    list(APPEND AVM_C_CONFIGS "CMAKE_C_FLAGS_${config}")
    list(APPEND AVM_CXX_CONFIGS "CMAKE_CXX_FLAGS_${config}")
    list(APPEND AVM_EXE_LINKER_CONFIGS "CMAKE_EXE_LINKER_FLAGS_${config}")
  endforeach()
else()
  string(TOUPPER ${CMAKE_BUILD_TYPE} config)
  set(AVM_C_CONFIGS "CMAKE_C_FLAGS_${config}")
  set(AVM_CXX_CONFIGS "CMAKE_CXX_FLAGS_${config}")
  set(AVM_EXE_LINKER_CONFIGS "CMAKE_EXE_LINKER_FLAGS_${config}")
endif()
...
set(AVM_C_PASSED_TESTS)
set(AVM_C_FAILED_TESTS)
set(AVM_CXX_PASSED_TESTS)
set(AVM_CXX_FAILED_TESTS)

function(avm_push_var var new_value)
  set(SAVED_${var}
      ${${var}}
      PARENT_SCOPE)
  set(${var}
...
# $AVM_C_PASSED_TESTS or $AVM_C_FAILED_TESTS depending on out come. When the
...
# test variables.
function(avm_check_c_compiles test_name test_source result_var)
  ...
  string(FIND "${AVM_C_PASSED_TESTS}" "${test_name}" C_TEST_PASSED)
  string(FIND "${AVM_C_FAILED_TESTS}" "${test_name}" C_TEST_FAILED)
  if(${C_TEST_PASSED} EQUAL -1 AND ${C_TEST_FAILED} EQUAL -1)
    unset(C_TEST_COMPILED CACHE)
    message("Running C compiler test: ${test_name}")
    check_c_source_compiles("${test_source} ${AVM_C_MAIN}" C_TEST_COMPILED)
    set(${result_var}
        ${C_TEST_COMPILED}
        PARENT_SCOPE)
    ...
    if(C_TEST_COMPILED)
      set(AVM_C_PASSED_TESTS
          "${AVM_C_PASSED_TESTS} ${test_name}"
          CACHE STRING "" FORCE)
    else()
      set(AVM_C_FAILED_TESTS
          "${AVM_C_FAILED_TESTS} ${test_name}"
          CACHE STRING "" FORCE)
...
# Confirms $test_source compiles and stores $test_name in one of
# $AVM_CXX_PASSED_TESTS or $AVM_CXX_FAILED_TESTS depending on out come. When the
# test passes $result_var is set to 1. When it fails $result_var is unset. The
# test is not run if the test name is found in either of the passed or failed
# test variables.
function(avm_check_cxx_compiles test_name test_source result_var)
  if(DEBUG_CMAKE_DISABLE_COMPILER_TESTS)
    return()
  endif()
  ...
  string(FIND "${AVM_CXX_PASSED_TESTS}" "${test_name}" CXX_TEST_PASSED)
  string(FIND "${AVM_CXX_FAILED_TESTS}" "${test_name}" CXX_TEST_FAILED)
  if(${CXX_TEST_PASSED} EQUAL -1 AND ${CXX_TEST_FAILED} EQUAL -1)
    ...
    check_cxx_source_compiles("${test_source} ${AVM_CXX_MAIN}"
                              CXX_TEST_COMPILED)
    set(${result_var}
        ${CXX_TEST_COMPILED}
        PARENT_SCOPE)
    ...
    if(CXX_TEST_COMPILED)
      set(AVM_CXX_PASSED_TESTS
          "${AVM_CXX_PASSED_TESTS} ${test_name}"
          CACHE STRING "" FORCE)
    else()
      set(AVM_CXX_FAILED_TESTS
          "${AVM_CXX_FAILED_TESTS} ${test_name}"
          CACHE STRING "" FORCE)
...
function(avm_check_source_compiles test_name test_source result_var)
  unset(C_PASSED)
  unset(CXX_PASSED)
  avm_check_c_compiles(${test_name} ${test_source} C_PASSED)
  avm_check_cxx_compiles(${test_name} ${test_source} CXX_PASSED)
...
function(avm_get_inline result)
