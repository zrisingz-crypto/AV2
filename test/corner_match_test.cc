...
namespace test_libavm {

namespace AV2CornerMatch {
...
class AV2CornerMatchTest : public ::testing::TestWithParam<CornerMatchParam> {
 public:
  ~AV2CornerMatchTest() override;
  void SetUp() override;
...
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(AV2CornerMatchTest);

AV2CornerMatchTest::~AV2CornerMatchTest() = default;
void AV2CornerMatchTest::SetUp() {
  rnd_.Reset(ACMRandom::DeterministicSeed());
  target_compute_mean_stddev_func = GET_PARAM(1);
  target_compute_corr_func = GET_PARAM(2);
}

void AV2CornerMatchTest::GenerateInput(uint8_t *input1, uint8_t *input2, int w,
                                       int h, int mode) {
  if (mode == 0) {
    for (int i = 0; i < h; ++i)
      for (int j = 0; j < w; ++j) {
...
void AV2CornerMatchTest::RunCheckOutput() {
  ...
  for (int i = 0; i < num_iters; ++i) {
    ...
    bool c_valid1 = avm_compute_mean_stddev_c(input1.get(), w, x1, y1, &c_mean1,
                                              &c_one_over_stddev1);
    bool c_valid2 = avm_compute_mean_stddev_c(input2.get(), w, x2, y2, &c_mean2,
                                              &c_one_over_stddev2);
    ...
    // avm_compute_mean_stddev(), and one_over_stddev is set to 0 instead.
    // This causes avm_compute_correlation() to return 0, without causing a
    // division by 0.
    const double c_corr = avm_compute_correlation_c(
...
void AV2CornerMatchTest::RunSpeedTest() {
  const int w = 16, h = 16;
  ...
  // Time avm_compute_mean_stddev()
  double c_mean1, c_one_over_stddev1, c_mean2, c_one_over_stddev2;
  avm_usec_timer_start(&ref_timer);
  for (int i = 0; i < num_iters; i++) {
    avm_compute_mean_stddev_c(input1.get(), w, 0, 0, &c_mean1,
                              &c_one_over_stddev1);
    avm_compute_mean_stddev_c(input2.get(), w, 0, 0, &c_mean2,
                              &c_one_over_stddev2);
  }
  avm_usec_timer_mark(&ref_timer);
  int elapsed_time_c = static_cast<int>(avm_usec_timer_elapsed(&ref_timer));
  ...
  int elapsed_time_simd = static_cast<int>(avm_usec_timer_elapsed(&test_timer));

  printf(
      "avm_compute_mean_stddev(): c_time=%6d   simd_time=%6d   "
      "gain=%.3f\n",
      elapsed_time_c, elapsed_time_simd,
      (elapsed_time_c / (double)elapsed_time_simd));

  // Time avm_compute_correlation
  avm_usec_timer_start(&ref_timer);
  for (int i = 0; i < num_iters; i++) {
    avm_compute_correlation_c(input1.get(), w, 0, 0, c_mean1,
                              c_one_over_stddev1, input2.get(), w, 0, 0,
                              c_mean2, c_one_over_stddev2);
  }
  avm_usec_timer_mark(&ref_timer);
  elapsed_time_c = static_cast<int>(avm_usec_timer_elapsed(&ref_timer));
  ...
  avm_usec_timer_mark(&test_timer);
  elapsed_time_simd = static_cast<int>(avm_usec_timer_elapsed(&test_timer));

  printf(
      "avm_compute_correlation(): c_time=%6d   simd_time=%6d   "
      "gain=%.3f\n",
      elapsed_time_c, elapsed_time_simd,
      (elapsed_time_c / (double)elapsed_time_simd));
}

TEST_P(AV2CornerMatchTest, CheckOutput) { RunCheckOutput(); }
TEST_P(AV2CornerMatchTest, DISABLED_Speed) { RunSpeedTest(); }

#if HAVE_SSE4_1
INSTANTIATE_TEST_SUITE_P(
    SSE4_1, AV2CornerMatchTest,
    ::testing::Values(make_tuple(0, &avm_compute_mean_stddev_sse4_1,
                                 &avm_compute_correlation_sse4_1),
                      make_tuple(1, &avm_compute_mean_stddev_sse4_1,
                                 &avm_compute_correlation_sse4_1)));
#endif

#if HAVE_AVX2
INSTANTIATE_TEST_SUITE_P(
    AVX2, AV2CornerMatchTest,
    ::testing::Values(make_tuple(0, &avm_compute_mean_stddev_avx2,
                                 &avm_compute_correlation_avx2),
                      make_tuple(1, &avm_compute_mean_stddev_avx2,
                                 &avm_compute_correlation_avx2)));
#endif
}  // namespace AV2CornerMatch
