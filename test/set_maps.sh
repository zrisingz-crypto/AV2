...
##
## This file tests the libavm set_maps example. To add new tests to this file,
## do the following:
...
set_maps_verify_environment() {
  ...
  if [ -z "$(avm_tool_path set_maps)" ]; then
    elog "set_maps not found. It must exist in LIBAVM_BIN_PATH or its parent."
    return 1
  fi
}
...
set_maps() {
  local encoder="$(avm_tool_path set_maps)"
  local codec="$1"
  local output_file="${AVM_TEST_OUTPUT_DIR}/set_maps_${codec}.ivf"
...
set_maps_av2() {
  if [ "$(av2_encode_available)" = "yes" ]; then
    set_maps av2 || return 1
  fi
}

set_maps_tests="set_maps_av2"
