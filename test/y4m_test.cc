...
const Y4mTestParam kY4mTestVectors[] = {
  { "park_joy_90p_8_420.y4m", 8, AVM_IMG_FMT_I420,
    "e5406275b9fc6bb3436c31d4a05c1cab" },
  { "park_joy_90p_8_420_monochrome.y4m", 8, AVM_IMG_FMT_I420,
    "95ef5bf6218580588be24a5271bb6a7f" },
  { "park_joy_90p_8_420_vertical_csp.y4m", 8, AVM_IMG_FMT_I420,
    "e5406275b9fc6bb3436c31d4a05c1cab" },
  { "park_joy_90p_8_422.y4m", 8, AVM_IMG_FMT_I422,
    "284a47a47133b12884ec3a14e959a0b6" },
  { "park_joy_90p_8_444.y4m", 8, AVM_IMG_FMT_I444,
    "90517ff33843d85de712fd4fe60dbed0" },
  { "park_joy_90p_10_420.y4m", 10, AVM_IMG_FMT_I42016,
    "63f21f9f717d8b8631bd2288ee87137b" },
  { "park_joy_90p_10_422.y4m", 10, AVM_IMG_FMT_I42216,
    "48ab51fb540aed07f7ff5af130c9b605" },
  { "park_joy_90p_10_444.y4m", 10, AVM_IMG_FMT_I44416,
    "067bfd75aa85ff9bae91fa3e0edd1e3e" },
  { "park_joy_90p_12_420.y4m", 12, AVM_IMG_FMT_I42016,
    "9e6d8f6508c6e55625f6b697bc461cef" },
  { "park_joy_90p_12_422.y4m", 12, AVM_IMG_FMT_I42216,
    "b239c6b301c0b835485be349ca83a7e3" },
  { "park_joy_90p_12_444.y4m", 12, AVM_IMG_FMT_I44416,
    "5a6481a550821dab6d0192f5c63845e9" },
};

static const int PLANES_YUV[] = { AVM_PLANE_Y, AVM_PLANE_U, AVM_PLANE_V };

class Y4mVideoSourceTest : public ::testing::TestWithParam<Y4mTestParam>,
                           public ::libavm_test::Y4mVideoSource {
 protected:
  ...
  void HeaderChecks(unsigned int bit_depth, avm_img_fmt_t fmt) {
...
class Y4mVideoWriteTest : public Y4mVideoSourceTest {
 protected:
  ...
  void WriteY4mAndReadBack() {
    ...
    tmpfile_ = new libavm_test::TempOutFile;
    ASSERT_TRUE(tmpfile_->file() != NULL);
    y4m_write_file_header(buf, sizeof(buf), kWidth, kHeight, &framerate,
                          img()->monochrome, img()->csp, y4m_.avm_fmt,
                          y4m_.bit_depth, AVM_CR_STUDIO_RANGE);
  ...
  libavm_test::TempOutFile *tmpfile_;
};
...
TEST(Y4MHeaderTest, RegularHeader) {
  libavm_test::TempOutFile f;
  fwrite(kY4MRegularHeader, 1, sizeof(kY4MRegularHeader), f.file());
  fflush(f.file());
  EXPECT_EQ(0, fseek(f.file(), 0, 0));

  y4m_input y4m;
  EXPECT_EQ(y4m_input_open(&y4m, f.file(), NULL, 0, AVM_CSP_UNSPECIFIED,
                           /*only_420=*/0),
            0);
  ...
  EXPECT_EQ(y4m.color_range, AVM_CR_STUDIO_RANGE);
...
TEST(Y4MHeaderTest, LongHeader) {
  libavm_test::TempOutFile tmpfile;
  FILE *f = tmpfile.file();
  ...
  y4m_input y4m;
  EXPECT_EQ(
      y4m_input_open(&y4m, f, NULL, 0, AVM_CSP_UNSPECIFIED, /*only_420=*/0), 0);
  EXPECT_EQ(y4m.pic_w, 4);
  EXPECT_EQ(y4m.pic_h, 4);
  EXPECT_EQ(y4m.fps_n, 30);
  EXPECT_EQ(y4m.fps_d, 1);
...
TEST(Y4MHeaderTest, FullRangeHeader) {
  ...
  EXPECT_EQ(y4m.interlace, 'p');
  EXPECT_EQ(strcmp("420jpeg", y4m.chroma_type), 0);
  EXPECT_EQ(y4m.color_range, AVM_CR_FULL_RANGE);
  y4m_input_close(&y4m);
}
...
TEST(Y4MHeaderTest, WriteStudioColorRange) {
  ...
  EXPECT_GE(y4m_write_file_header(
                ...
                /*monochrome=*/0, AVM_CSP_UNSPECIFIED, AVM_IMG_FMT_I420,
                /*bit_depth=*/8, AVM_CR_STUDIO_RANGE),
            0);
